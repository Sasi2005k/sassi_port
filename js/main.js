...
(function () {
    ...
    function loadDataObj() {
        var lsData = null;
        try {
            lsData = JSON.parse(localStorage.getItem(STORE_KEY));
        } catch(e) {}
    ...
    /* ----------------------------------------------------------
       9. MASTER SAVE BUTTON & EXPORT
    ---------------------------------------------------------- */
    var btnExport = document.getElementById('btn-export');
    if(btnExport) {
        btnExport.addEventListener('click', function () {
            // Ensure they save before exporting
            alert("If you have unsaved changes, click 'Save Locally' first!");
            var data = loadDataObj();
            var jsContent = "window.PORTFOLIO_DATA = " + JSON.stringify(data, null, 4) + ";\n";
            ...
            showToast('data.js downloading... Now push this file to GitHub!');
            closeModal();
        });
    }
